...
class FinnHubScraper:
    ...
    def __init__(self, start_date=None, end_date=None, tickers=None):
        ...
        self.start_date = start_date if start_date else (datetime.now() - timedelta(days=7)).strftime('%Y-%m-%d')
        ...
        self.finhub_key = os.getenv('FINHUB_API_KEY')
    ...
    def store_news(self, news_data):
        ...
        for news in news_data:
            if detect(news['headline']) == 'en':
                ...
                self.scraped_news.append(news)  # Append news to the list instead of storing in a database
