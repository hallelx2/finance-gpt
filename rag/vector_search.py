...
from document_retriever import FinnHubScraper
...
def bulk_check_duplicates(document_urls: List[str], collection_name: str = 'financenews_documents') -> List[str]:
    """
...
def store_documents(search_tickers: List[str]):
    ...
    non_duplicate_docs = [
        DocumentModel(**doc) for doc in scraped_documents if doc["url"] not in duplicate_urls
    ]
    ...
    if non_duplicate_docs:
        ...
        for doc, doc_id in tqdm(
            zip(vector_documents, uuids), desc="Embedding documents", unit="doc", total=len(vector_documents)
        ):
